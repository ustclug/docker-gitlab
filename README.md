[![](https://images.microbadger.com/badges/image/ustclug/gitlab.svg)](http://microbadger.com/images/ustclug/gitlab "Get your own image badge on microbadger.com")
[![Build Status](https://travis-ci.org/ustclug/docker-gitlab.svg?branch=master)](https://travis-ci.org/ustclug/docker-gitlab)

# ustclug/gitlab:9.1.0-1
...
```bash
docker pull ustclug/gitlab:latest
```
...
docker run --name gitlab -d [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:sanitize
```
...
| `NGINX_BEHIND_REVPROXY` | Configure the internal nginx to restore original client ip if GitLab runs behind a reverse proxy. Defaults to `false`. |
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:9.1.0-1 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `ustclug/gitlab:9.1.0-1` from `sameersbn/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `sameersbn/gitlab:8.0.5-1` before upgrading to `sameersbn/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    ustclug/gitlab:x.x.x app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -d [OPTIONS] ustclug/gitlab:9.1.0-1
```
